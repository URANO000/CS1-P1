...
public class Sistema {
    ...
    public void ordenarLibro() {
        ...
        for (Libro libro : libros) {
            System.out.println(libro.getTitulo() + " de " + libro.getAutor().getNombre());  //Here what I'm doing is showing what I did in the comparator, to show the user
        }
    }
    ...
    public void devolverLibro(Usuario usuario, Libro libro) throws Exception{
        if (!prestamos.containsKey(usuario)) {
            throw new Exception("El usuario no tiene libros prestados.");
        }

        if (!prestamos.get(usuario).contains(libro)) {
            throw new Exception("El usuario no ha prestado este libro.");
        }

        prestamos.get(usuario).remove(libro);
        libro.setDisponible(true); // Book is available again
...
//    //This is for exception management of inputs since the try and catch isn't working for some reason I can't figure out
//    private static String inputValido(Scanner sc, String prompt) {
//        String input;
//        while (true) {
//            System.out.print(prompt);
//            input = sc.nextLine().trim();
//
//            // This part basically checks if everything is filled out and not a number
//            if (!input.isEmpty() && !input.matches("\\d+")) {  //These all come from java.lang.String (String classs)
//                return input;
//            }
//
//            // Give appropriate feedback to the user
//            if (input.matches("\\d+")) {
//                System.out.println("El valor no puede ser numérico");
//            } else {
